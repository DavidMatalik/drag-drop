...
exports.makeDraggable = (element, boundary) => {
...
2. Optional: Create a boundary element (if boundary should be sth. else then body)
3. Pass created content from 1-2 as arguments into makeDraggable.
4. Define a class in css .absolute {position: absolute} 
5. The element can then be dragged around and stays where the mouse goes up.
*/

/* ***************** Explanation of makeDraggable Code *****************
1. absolute class makes element's position absolute and changes mouse cursor
   when mouse hovers over element (You must create)
...
exports.makeDroppable = (draggable, dropTarget, doOndrop, boundary=document.querySelector('body')) => {
...
4. Optional: Create a boundary element (if boundary should be sth. else then body)
5. Pass created content from 1-4 as arguments into makeDroppable.
5. Create a css class 'drop-effect' -
